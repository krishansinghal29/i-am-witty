...
/**
 * riffy brand marks — the "r-spark" glyph + "riffy" wordmark.
 *
 * The vector paths are copied directly from design/logo/svg (riffy-mark.svg,
 * riffy-icon.svg, riffy-wordmark.svg) so the app carries its own copy and never
 * reaches outside the frontend for brand assets. The wordmark is Fredoka 600
 * converted to outlines, so it needs no font to render.
 */

export type BrandTone = 'gradient' | 'ink' | 'white' | 'solid';

const WARM_FROM = '#F97316';
const WARM_TO = '#F7C13B';

function glyphFill(tone: BrandTone, gradientId: string): string {
  switch (tone) {
    case 'gradient':
...
/** The standalone r-spark glyph (no tile). */
export function RiffyMark({
  size = 28,
  tone = 'gradient',
  title = 'riffy',
  style,
      ...
      {tone === 'gradient' && (
        <defs>
          <linearGradient id={gid} x1="0" y1="0" x2="1" y2="1">
            ...
            <stop offset="1" stopColor={WARM_TO} />
          </linearGradient>
        </defs>
      )}
      <g fill={fill}>
...
/** The app-icon tile: warm gradient squircle with the white glyph. */
export function RiffyIcon({
  size = 56,
  radius = 0.23,
  title = 'riffy',
  style,
  ...
  size?: number;
  /** Corner radius as a fraction of the size (brand spec: 23%). */
  radius?: number;
  title?: string;
  style?: CSSProperties;
}) {
  ...
  return (
    ...
    >
      ...
      <rect width="100" height="100" rx={radius * 100} fill={`url(#${gid})`} />
      <g fill="#fff">
        <rect x="35.5" y="35" width="11.5" height="35" rx="5.75" />
        <path
          d="M42 46 C45 38 52 36.5 60 40.5"
...
/** The outlined "riffy" wordmark. */
export function RiffyWordmark({
  height = 28,
  tone = 'ink',
  title = 'riffy',
  style,
  ...
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 -76 206 104"
      height={height}
      width={(height * 206) / 104}
      role="img"
      aria-label={title}
      style={style}
    >
...
/** Horizontal lockup: the gradient icon tile beside the wordmark. */
export function RiffyLockup({
  height = 36,
  tone = 'ink',
  style,
}: {
  /** Drives the icon tile size; the wordmark scales to match. */
  height?: number;
  tone?: BrandTone;
  style?: CSSProperties;
}) {
  return (
    <span
      ...
      aria-label="riffy"
    >
      <RiffyIcon size={height} title="" />
      <RiffyWordmark height={height * 0.62} tone={tone} title="" />
    </span>
  );
}
