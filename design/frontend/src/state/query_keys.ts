 ...
 *
 * NOTE: progress, plan, and onboarding status all come from the `home` query
 * (the backend bundles them into a single /v1/home response). There is NO
 * separate progress or plan key. Free-limit data is returned inline by the
 * start/complete mutations and is therefore NOT tracked by a query key.
 *
